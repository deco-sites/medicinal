import { isSchedulingActive } from 'site/sdk/contentScheduling.ts';
...
export const loader = async ({ title, promotions = [] }: Props, request: Request, context: AppContext) => {
    const filteredPromotions = await filterByMatcher({
        items: promotions,
        context,
        request,
    });
    return { title, promotions: filteredPromotions.filter(({ notActive }) => !notActive) };
};
export default function PromotionsSection({ title, promotions }: SectionProps<typeof loader>) {
    ...
    return (<div class='flex flex-col gap-6 h-full w-full rounded-xl border border-light-gray p-6 md:p-16'>
            {title && (<h2 class='text-dark text-base font-extrabold uppercase'>
                    {title}
                </h2>)}

            {promotions.map((promotion, index) => {
            console.log('promotion', promotion);
            const { cta, ctaLink, ctaCopyButton, image, ruleDateHourFormatted, showRuleDateHourFormatted, showCtaButton, showCtaCopyButton, showImage, showRuleTextPromotion, showTag, showText, showTitle, tag, text, titlePromotion, ruleTextPromotion, } = promotion;
            const isCopied = copiedIndex === index;
            return (<div key={index} class='flex flex-col h-full w-full rounded-xl border border-light-gray p-6 gap-4'>
                        <div class='flex gap-4 flex-col lg:flex-row'>
                            {!showImage && image && (<Image src={image} width={200} height={200} class='max-h-[200px] max-w-[200px]'/>)}
                            <div>
                            </div>
                            <div class='flex flex-col gap-5 relative lg:w-[663px]'>
                                <div class='flex flex-col gap-[6px]'>
                                    {!showTitle && (<p class='text-dark font-lemon text-sm font-bold uppercase'>
                                            {titlePromotion}
                                        </p>)}
                                    {!showRuleDateHourFormatted && ruleDateHourFormatted && (<p class='text-gray text-xs font-normal'>Validade: {ruleDateHourFormatted}</p>)}
                                </div>
                                {!showTag && (<div class='max-lg:max-w-[140px] whitespace-nowrap min-w-[120px] h-[21px] p-[6px] absolute right-0 top-0 bg-gradient-to-r text-transparent bg-clip-text border border-red uppercase text-xs font-semibold flex items-center rounded-[5px] from-red from-35% to-orange to-90% justify-center max-lg:static'>
                                        {tag}
                                    </div>)}

                                {!showText && text && (<div class='text-sm text-dark font-normal' dangerouslySetInnerHTML={{ __html: text }}/>)}

                                <div class='flex gap-4 flex-col lg:flex-row'>
                                    {!showCtaButton && (<div class='group'>
                                            <a class='flex h-10 items-center gap-2 justify-center font-bold text-[11px] md:text-[13px] leading-[14px] md:leading-[17.5px] uppercase  md:h-[40px] rounded-full font-lemon-milk text-dark group-first:text-white group-first:bg-gradient-to-r from-red from-35% to-orange to-90% group-first:px-2 md:group-first:px-6 group-hover:bg-gradient-to-r transition-all group-hover:text-transparent group-hover:bg-clip-text border border-transparent group-first:hover:border-red' href={ctaLink}>
                                                {cta}
                                                <Icon id='ArrowRight' size={20} class='text-white group-hover:text-red'/>
                                            </a>
                                        </div>)}
                                    {!showCtaCopyButton && ctaCopyButton && (<div onClick={() => {
                        ...
                        {!showRuleTextPromotion && (<div class='flex p-[10px] justify-center items-center rounded-[3px] bg-ice'>
                                <p class='text-[#696968] font-normal text-xs'>{ruleTextPromotion}</p>
                            </div>)}
                    </div>);
        })}
        </div>);
