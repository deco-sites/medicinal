...
function BannerInfoContainer({ banner, preload, isMobile, content, textPosition }: Props) {
    ...
    return (<Wrapper {...props} class='w-full block max-w-[1448px] mx-auto relative px-[17px] md:px-10'>
			<Picture preload={preload}>
				<Source media='(max-width: 767px)' fetchPriority={preload ? 'high' : 'auto'} src={mobileSrc} width={360} height={mobileHeight}/>
				<Source media='(min-width: 768px)' fetchPriority={preload ? 'high' : 'auto'} src={desktopSrc} width={1440} height={600}/>
				<img class='object-cover w-full rounded-[35px]' loading={preload ? 'eager' : 'lazy'} src={desktopSrc} alt={alt} style={{ height: `${height}px` }}/>
			</Picture>

			<div class={`absolute top-0 w-full h-full flex items-end md:items-center right-0 left-0 px-[30px] md:px-20 lg:px-[126px] text-center justify-center
        ${textPosition && textPositionMapping[textPosition]}`}>
				<div class='flex flex-col items-center md:items-start mb-10 md:mb-0'>
					{content?.tag &&
            (<span class={`font-inter font-medium mb-6 
              ...
              `}>
					...
					{content?.image &&
            (<Image src={content?.image.src || ''} width={content?.image.width || 0} height={content?.image.height || 0} alt='' class={'mb-6'}/>)}

					{content?.tag?.divider && <div class='w-full max-w-[475] border-b border-light-gray mb-8'/>}

					{content?.title &&
            (<RenderHTML html={content.title} class={`font-lemon-milk font-bold text-ice mb-8 
                ${content.titleSize
                    ? titleSizeMapping[content.titleSize]
                    : 'text-[24px] md:text-[40px] leading-[24px] md:leading-[42px]'} ${content.centerTitleOnMobile ? 'text-center md:text-left' : ''}`}/>)}

					{content?.description &&
            (<RenderHTML html={content.description} class='font-inter text-sm md:text-[16px] font-medium leading-[22px] md:leading-[27px] text-ice mb-8'/>)}

					{content?.cta && (<span class='flex items-center gap-[10px] uppercase font-lemon-milk font-bold text-[13px] leading-[17px] text-ice py-[15px] px-6 rounded-full 
                ...
                border-transparent hover:border-red hover:fontWithGradient cursor-pointer max-h-[40px]'>
							{content.cta.text}
							<Icon id='ArrowRight' size={16} class='text-white group-hover:text-red'/>
						</span>)}
				</div>
			</div>
		</Wrapper>);
