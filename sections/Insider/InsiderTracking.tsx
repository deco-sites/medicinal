	...
	const line_items = orderForm.items.map((item) => {
		return {
			product: {
				...
				url: `${globalThis.window.location.origin}${item.detailUrl}`,
				product_image_url: item.imageUrl,
			},
			subtotal: parseFloat(
				((item.sellingPrice * item.quantity) / 100).toFixed(2),
			),
...
export const extractProductData = (
	...
	const SALE_PRICE = product.offers?.offers[0].priceSpecification.find(
		(priceSpecification) =>
			priceSpecification.priceType === 'https://schema.org/SalePrice' &&
			priceSpecification.priceComponentType !==
				'https://schema.org/Installment',
	)?.price ?? 0

	const PIX_PRICE = parseFloat(
		(SALE_PRICE - SALE_PRICE * PIX_DISCOUNT).toFixed(2),
	)
	...
	return {
		...
		unit_price: product.offers?.offers[0].priceSpecification.find(
			(priceSpecification) => priceSpecification.priceType === 'https://schema.org/ListPrice',
		)?.price,
...
const InsiderTrackingSection = ({ pageType, data }: InsiderTrackingProps) => {
	const scriptContent = async (
		_id: string,
		{ pageType, data }: InsiderTrackingProps,
	) => {
		const getUser = async () => {
			try {
		...
		const setInsiderObject = (
			...
			if (pageType === 'Product') {
				// @ts-ignore .
				globalThis.window.insider_object.product = data
			}
			if (pageType === 'Category' || pageType === 'Search') {
				// @ts-ignore .
				globalThis.window.insider_object.listing = { items: data }
			}
		}
	...
	return (
		<script
			src={scriptAsDataURI(scriptContent, 'insider-tracking', {
				pageType,
				data,
			})}
		/>
