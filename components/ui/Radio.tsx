...
export default function Radio(
		...
		return (
			<div
				class={`flex items-center justify-between border-2 rounded-md px-4 py-4 sm:py-5 ${
					selected === value ? 'border-green bg-white' : 'border-light-gray-200 bg-ice'
				}`}
			>
	...
	if (type === 'product') {
		return (
			<div
				class={`flex items-center justify-between border-2 rounded-md px-4 ${
					selected === value ? 'border-green bg-white' : 'border-light-gray-200 bg-ice'
				}`}
			>
				<div class='flex items-center gap-2 w-full'>
					<div class='custom-radio py-3'>
						<input
							id={value}
					...
					<label
						for={value}
						class='text-xs sm:text-sm w-full py-3 cursor-pointer'
						dangerouslySetInnerHTML={{ __html: text }}
					/>
				</div>
			</div>
