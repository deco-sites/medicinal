...
export default function Benefits(
	...
	const listOfBenefits = benefits.map((benefit, index) => {
		...
		const benefitLayout = !layout?.variation || layout?.variation === 'Simple' ? 'tiled' : 'piledup'

		return (
			<div
				class={`${reverse ? 'bg-primary text-primary-content p-4 lg:px-8 lg:py-4' : ''} flex gap-4 ${
			...
			>
				...
				<div class='flex-auto flex flex-col gap-1 lg:gap-2'>
					<div
						class={`text-base lg:text-xl leading-7 ${reverse ? 'text-base-100' : 'text-base-content'}`}
					>
						{benefit.label}
					</div>
					<p
						class={`text-sm leading-5 ${reverse ? 'text-base-100' : 'text-neutral'} ${
