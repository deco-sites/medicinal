...
function BannerInfoWithDivider({ banner, preload, isMobile, content }: Props) {
    ...
    return (<div class='w-full relative'>
			<Picture preload={preload}>
				<Source media='(max-width: 767px)' fetchPriority={preload ? 'high' : 'auto'} src={mobileSrc || desktopSrc} width={360} height={mobileHeight || 600}/>
				<Source media='(min-width: 768px)' fetchPriority={preload ? 'high' : 'auto'} src={desktopSrc} width={1440} height={desktopHeight || 600}/>
				<img class='object-cover w-full' loading={preload ? 'eager' : 'lazy'} src={desktopSrc} alt={alt} style={{ height: heightStyle }}/>
			</Picture>
