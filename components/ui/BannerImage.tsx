...
function BannerImage({ desktopSrc, desktopHeight, mobileSrc, mobileHeight, alt, preload, isContainer, rounded, isMobile, }: Props) {
    const heightStyle = isMobile
        ? (mobileHeight ? `${mobileHeight}px` : 'auto')
        : (desktopHeight ? `${desktopHeight}px` : 'auto');
    return (<div class={`w-full mx-auto ${isContainer && 'max-w-[1440px]'}`}>
