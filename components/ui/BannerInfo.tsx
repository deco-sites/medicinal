...
function BannerInfo({ banner, preload, isMobile, rounded, content, textPosition }: Props) {
    ...
    return (<div class='w-full relative'>
			<Picture preload={preload}>
				<Source media='(max-width: 767px)' fetchPriority={preload ? 'high' : 'auto'} src={mobileSrc || desktopSrc} width={360} height={mobileHeight}/>
          ...
          `} loading={preload ? 'eager' : 'lazy'} src={desktopSrc} alt={alt} style={{ height: heightStyle }}/>
			</Picture>

			<div class={`absolute top-0 w-full max-w-[1440px] mx-auto h-full flex items-center right-0 left-0 px-[30px] md:px-20 lg:px-[152px]
        ${textPosition ? textPositionMapping[textPosition] : 'text-start justify-start'}`}>
				<div class={`flex flex-col w-full 
        ${textPosition === 'center' ? 'items-center md:max-w-[50%]' : 'items-start max-w-[390px]'}`}>
					{content?.tag &&
            ...
            (<p class='font-inter text-sm md:text-[16px] font-medium leading-[22px] md:leading-[27px] text-ice mb-8'>
					...
					{content?.cta && (<a href={content.cta.link} class='flex items-center gap-[10px] uppercase font-lemon-milk font-bold text-[13px] leading-[17px] text-ice py-[15px] px-6 rounded-full 
